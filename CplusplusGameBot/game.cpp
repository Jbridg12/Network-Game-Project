...
list<u_int*> curr_playing_inputs;
list<u_int*> curr_placing_inputs;

internal void init_game(const Chromosome& x)
{
	// Initialize
	end_of_game = false;
	all_game_blocks = {};
	...
	// Store Chromosome lists as local copies
	playing_inputs = x.playing_inputs;
	placing_inputs = x.placing_inputs;
	curr_playing_inputs = playing_inputs.front();
	curr_placing_inputs = placing_inputs.front();
	playing_inputs.pop_front();
	...
	switch (current_gamemode)
	{
		...
		*/
		case (GAMEMODE::Playing):
			...
			{
				input_timer = 0;
				if (curr_playing_inputs.size())
				{
					u_int* ip = curr_playing_inputs.front();
					curr_playing_inputs.pop_front();
		...
		*/
		case (GAMEMODE::Placing):
			...
			if (input_timer >= INPUT_TIMER)
			{
				...
				// Genetic Algorithm Input
				if (curr_placing_inputs.size())
				{
					u_int* ip = curr_placing_inputs.front();
					curr_placing_inputs.pop_front();
